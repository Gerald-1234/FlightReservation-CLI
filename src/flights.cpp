...
void Flight::initializeSeats(int seatCount) {
	seats.clear();
	seats.reserve(static_cast<std::size_t>(seatCount));
...
bool Flight::isValidDateTimeFormat(const std::string& dateTime) {
	// Structural check for "YYYY-MM-DD HH:MM". Mirrors the former regex
	// ^\d{4}-\d{2}-\d{2} \d{2}:\d{2}$ without <regex>, which currently
	// miscompiles when pulled in through `import std;` on GCC.
	if (dateTime.size() != 16) {
		return false;
	}
	for (std::size_t i = 0; i < dateTime.size(); ++i) {
		const char c = dateTime[i];
		switch (i) {
			case 4: case 7:
				if (c != '-') return false;
				break;
...
bool Flight::bookSeats(int seatCount) {
	...
	toBook.reserve(static_cast<std::size_t>(seatCount));
